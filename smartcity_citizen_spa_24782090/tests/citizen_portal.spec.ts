...
test.describe('Modul 1: Otorisasi & Sesi (AUTH-04, AUTH-05, AUTH-06)', () => {
    ...
    test('AUTH-04: Akses #dashboard tanpa token → redirect ke #login', async ({ page }) => {
        ...
        // -------------------------------------------------------------------
        // Pastikan tidak ada sisa state yang dapat mengganggu redirect
        // - Hapus cookie konteks
        // - Pastikan localStorage kosong
        // - Tangani dialog/alert yang mungkin muncul
...
test.describe('Modul 5: Interaktivitas UI (UI-01 through UI-06)', () => {
    ...
    // LANGKAH 1: Setup Mocking API
    // Pastikan URL di sini ('**/api/dashboard/stats/') sama dengan yang ada di app.js Anda
    // Sesuaikan dengan URL yang dipanggil di app.js (loadSummaryStats)
    await page.route('**/api/report/?tab=my_reports&page_size=1000', async (route) => {
        await route.fulfill({
            status: 200,
            contentType: 'application/json',
            body: JSON.stringify({
                results: [
                    { id: 1, status: 'DRAFT', title: 'Test 1' },
                    { id: 2, status: 'RESOLVED', title: 'Test 2' }
                ]
            })
        });
    });

    await page.route('**/api/report/?tab=my_reports&page=1', async (route) => {
        await route.fulfill({
            status: 200,
            contentType: 'application/json',
            body: JSON.stringify({
                ...
                previous: null,
                results: [
                    { id: 1, status: 'DRAFT', title: 'Test 1', category: 'Infrastruktur', location: 'Bandung', updated_at: '2026-07-02T08:00:00Z', description: 'Deskripsi 1', reporter_name: 'Warga A', is_owner: true },
                    { id: 2, status: 'RESOLVED', title: 'Test 2', category: 'Kebersihan', location: 'Bandung', updated_at: '2026-07-01T10:00:00Z', description: 'Deskripsi 2', reporter_name: 'Warga B', is_owner: false }
                ]
            })
        });
    });
    ...
    // PASTIKAN path di loginAdmin sudah benar (misal: '/admin/' atau '/auth/')
    await page.goto(`${SPA_URL}#login`);
    await setupAuthTokens(page, VALID_ACCESS_TOKEN, EXPIRED_REFRESH_TOKEN);

    // LANGKAH 3: Navigasi ke Dashboard menggunakan Hash
    await page.goto(`${SPA_URL}#dashboard`);

    // LANGKAH 4: Tunggu Dashboard ter-render
    await page.waitForSelector('#openReportModalBtn', { state: 'visible' });

    // LANGKAH 5: Verifikasi Canvas
    await expect(page.locator('#statusChart')).toBeVisible();
    await expect(page.locator('#categoryChart')).toBeVisible();

    // LANGKAH 6: Verifikasi Chart Instance (Internal Chart.js)
    // Tunggu hingga Chart.js instances terdaftar (race-safe)
    await page.waitForFunction(() =>
        window.Chart && Object.keys(window.Chart.instances || {}).length >= 2,
        { timeout: 10000 }
    );

    // LANGKAH 7: Verifikasi Tabel
    await expect(page.locator('#reportedTable')).toBeVisible();
    await expect(page.locator('#resolvedTable')).toBeVisible();

    console.log('[UI-01] ✅ Tes berhasil');
});
    ...
    // Mock endpoint search agar test tidak bergantung pada server nyata.
    await page.route('**/search/**', async (route) => {
        ...
        await route.fulfill({
            ...
            contentType: 'application/json',
            body: JSON.stringify({
                results: query.toLowerCase().includes('lampu')
                    ? [{ title: 'Lampu Jalan Rusak', category: 'Infrastruktur', location: 'Jl. Merdeka', status: 'REPORTED' }]
                    : []
            })
        });
    });

    // Pastikan elemen search muncul
    const searchInput = page.locator('#searchInput');
    await expect(searchInput).toBeVisible({ timeout: 10000 });

    const searchKeyword = 'Lampu';

    // 3. Gunakan Promise.all untuk menunggu respons yang benar sambil memicu Enter.
    const [response] = await Promise.all([
        page.waitForResponse(res => 
            res.url().includes(`/search/?q=${encodeURIComponent(searchKeyword)}`) && 
            res.status() === 200
        ),
    ...
    // 5. Verifikasi DOM Terupdate
    // Gunakan locator yang spesifik ke baris tabel untuk memastikan render selesai
    const tableRows = page.locator('#reportTableBody tr');
    
    if (responseData.results?.length > 0) {
        // Tunggu sampai jumlah baris sesuai dengan data hasil search
        await expect(tableRows).toHaveCount(responseData.results.length, { timeout: 5000 });
        
        // Verifikasi teks dalam baris pertama mengandung kata kunci
        await expect(tableRows.first()).toContainText(searchKeyword);
    } else {
        // Jika data kosong, pastikan pesan "Tidak ada hasil" muncul
        await expect(page.locator('#reportTableBody')).toContainText('Tidak ada hasil');
    }

    console.log('[UI-02] ✅ Live search berfungsi dengan baik');
});
    ...
    test('UI-05: Isi form dan simpan draft → modal tutup, notifikasi muncul', async ({ page }) => {
        ...
        const draftBadge = summaryStats.locator('.badge.bg-warning').first();
